...
import type { RawItem, MapCluster } from '../../../Domain/Types/mapTypes';


function clusterRadius(zoom: number): number {
    if (zoom >= 19) return 0;
    if (zoom >= 18) return 0.0002;
    if (zoom >= 17) return 0.0005;
    if (zoom >= 16) return 0.001;
    if (zoom >= 15) return 0.002;
    return 0.005;
}

export function clusterItems(items: RawItem[], zoom: number): MapCluster[] {
    const radius = clusterRadius(zoom);
    const used   = new Set<string>();
    const result: MapCluster[] = [];

    for (const item of items) {
        if (used.has(item.id) || !item.location) continue;
        used.add(item.id);
        const group: RawItem[] = [item];

        if (radius > 0) {
            for (const other of items) {
                if (used.has(other.id) || !other.location) continue;
                const dlat = Math.abs(other.location.lat - item.location.lat);
                const dlng = Math.abs(other.location.lng - item.location.lng);
        ...
        const lostCount  = group.filter(i => i.status?.includes("گم")).length;
        const foundCount = group.length - lostCount;
        result.push({ lat, lng, lostCount, foundCount, items: group });
    }
    return result;
}


export function pinSizeFromZoom(zoom: number): number {
...
export function makeClusterIcon(lostCount: number, foundCount: number, size: number): L.DivIcon {
