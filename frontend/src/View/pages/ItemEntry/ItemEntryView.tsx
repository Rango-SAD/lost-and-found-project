...
function ItemEntryView() {
  ...
  const toggleStatus = () => setIsFound(!isFound);
  ...
  const handleSubmit = async () => {
    if (!itemName.trim() || !tag.trim() || !category || !description.trim() || !photo) {
      setErrorMessage("لطفاً تمام فیلدها و عکس را وارد کنید");
      setShowError(true);
    ...
    setErrorMessage("✓ آیتم با موفقیت ثبت شد (ذخیره در مرورگر)");
    setShowError(true);
    setTimeout(() => {
      setShowError(false);
      handleCancel();
  ...
  const handleCancel = () => {
    setItemName(""); setTag(""); setCategory(""); setDescription("");
    setPhoto(null); setPhotoPreview(""); setIsFound(false);
  };
  ...
  return (
    ...
    >
      ...
      {showError && <div className={`error-popup ${errorMessage.includes('✓') ? 'success' : ''}`}>{errorMessage}</div>}

      <div className="main-grid">
        <div className="form-column">
          <div className="neon-input-card"><input type="text" placeholder="نام آیتم" className="glass-input" value={itemName} onChange={(e) => setItemName(e.target.value)} /></div>
          <div className="neon-input-card"><input type="text" placeholder="تگ" className="glass-input" value={tag} onChange={(e) => setTag(e.target.value)} /></div>
          <div className="neon-input-card relative">
            <select className="glass-input appearance-none cursor-pointer" value={category} onChange={(e) => setCategory(e.target.value)}>
          ...
          <div className="neon-input-card"><textarea placeholder="توضیحات...." rows={6} className="glass-input resize-none" value={description} onChange={(e) => setDescription(e.target.value)} /></div>
          <div className="status-container">
            <div className={`toggle-switch ${isFound ? 'active' : ''}`} onClick={toggleStatus}><div className="toggle-dot"></div></div>
        ...
        <div className="media-column">
          <div className="form-map-preview group" onClick={() => navigate("/map")}>
            <div className="h-full w-full opacity-60 group-hover:opacity-100 transition-opacity"><UniversityMap /></div>
          ...
          <div className="neon-input-card flex flex-col items-center justify-center p-14 border-dashed border-white/10 relative cursor-pointer hover:border-white/30 transition-all">
            ...
            {photoPreview ? (
              ...
              <><div className="text-white/20 text-4xl mb-2">📷</div><span className="text-white/30 text-[10px] uppercase tracking-widest">Upload Photo</span></>
            )}
          </div>
        </div>
      </div>
